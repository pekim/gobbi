...
type Record struct {
	...
	IsGtypeStructFor string       `xml:"http://www.gtk.org/introspection/glib/1.0 is-gtype-struct-for,attr"`
...
func (r *Record) init(ns *Namespace) {
	...
	if r.IsGtypeStructFor != "" {
		forRecord, ok := r.Namespace.recordOrClassRecordForName(r.IsGtypeStructFor)
		if ok {
			forRecord.gtypeStruct = r
		} else {
			forInterface, ok := r.Namespace.interfaceForName(r.IsGtypeStructFor)
			if ok {
				forInterface.gtypeStruct = r
			} else {
				panic(fmt.Sprintf("No gtype struct %s found for %s", r.IsGtypeStructFor, r.Name))
			}
		}
	}
}
