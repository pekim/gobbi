...
/*
	...
	void window_activateHandler(GObject *, gpointer);

	static gulong Window_signal_connect_activate(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "activate", G_CALLBACK(window_activateHandler), data);
	}

*/
/*

	void window_createHandler(GObject *, gpointer);

	static gulong Window_signal_connect_create(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "create", G_CALLBACK(window_createHandler), data);
	}

*/
/*

	void window_deactivateHandler(GObject *, gpointer);

	static gulong Window_signal_connect_deactivate(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "deactivate", G_CALLBACK(window_deactivateHandler), data);
	}

*/
/*

	void window_destroyHandler(GObject *, gpointer);

	static gulong Window_signal_connect_destroy(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "destroy", G_CALLBACK(window_destroyHandler), data);
	}

*/
/*

	void window_maximizeHandler(GObject *, gpointer);

	static gulong Window_signal_connect_maximize(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "maximize", G_CALLBACK(window_maximizeHandler), data);
	}

*/
/*

	void window_minimizeHandler(GObject *, gpointer);

	static gulong Window_signal_connect_minimize(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "minimize", G_CALLBACK(window_minimizeHandler), data);
	}

*/
/*

	void window_moveHandler(GObject *, gpointer);

	static gulong Window_signal_connect_move(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "move", G_CALLBACK(window_moveHandler), data);
	}

*/
/*

	void window_resizeHandler(GObject *, gpointer);

	static gulong Window_signal_connect_resize(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "resize", G_CALLBACK(window_resizeHandler), data);
	}

*/
/*

	void window_restoreHandler(GObject *, gpointer);

	static gulong Window_signal_connect_restore(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "restore", G_CALLBACK(window_restoreHandler), data);
	}
...
type DocumentSignalPageChangedCallback func(targetObject *Document, pageNumber int32)
...
func document_pageChangedHandler(c_targetObject *C.GObject, c_page_number C.gint, data C.gpointer) {
	...
	callback(targetObject, pageNumber)
}
...
type ValueSignalValueChangedCallback func(targetObject *Value, value float64, text string)
...
func value_valueChangedHandler(c_targetObject *C.GObject, c_value C.gdouble, c_text *C.gchar, data C.gpointer) {
	...
	targetObject := ValueNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalValueValueChangedMap[index].callback
	callback(targetObject, value, text)
}
...
type signalWindowActivateDetail struct {
	callback  WindowSignalActivateCallback
	handlerID C.gulong
}

var signalWindowActivateId int
var signalWindowActivateMap = make(map[int]signalWindowActivateDetail)
var signalWindowActivateLock sync.RWMutex

// WindowSignalActivateCallback is a callback function for a 'activate' signal emitted from a Window.
type WindowSignalActivateCallback func(targetObject *Window)

/*
ConnectActivate connects the callback to the 'activate' signal for the Window.

The returned value represents the connection, and may be passed to DisconnectActivate to remove it.
*/
func (recv *Window) ConnectActivate(callback WindowSignalActivateCallback) int {
	signalWindowActivateLock.Lock()
	defer signalWindowActivateLock.Unlock()

	signalWindowActivateId++
	instance := C.gpointer(recv.native)
	handlerID := C.Window_signal_connect_activate(instance, C.gpointer(uintptr(signalWindowActivateId)))

	detail := signalWindowActivateDetail{callback, handlerID}
	signalWindowActivateMap[signalWindowActivateId] = detail

	return signalWindowActivateId
}

/*
DisconnectActivate disconnects a callback from the 'activate' signal for the Window.

The connectionID should be a value returned from a call to ConnectActivate.
*/
func (recv *Window) DisconnectActivate(connectionID int) {
	signalWindowActivateLock.Lock()
	defer signalWindowActivateLock.Unlock()

	detail, exists := signalWindowActivateMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWindowActivateMap, connectionID)
}

//export window_activateHandler
func window_activateHandler(c_targetObject *C.GObject, data C.gpointer) {
	signalWindowActivateLock.RLock()
	defer signalWindowActivateLock.RUnlock()

	targetObject := WindowNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalWindowActivateMap[index].callback
	callback(targetObject)
}

type signalWindowCreateDetail struct {
	callback  WindowSignalCreateCallback
	handlerID C.gulong
}

var signalWindowCreateId int
var signalWindowCreateMap = make(map[int]signalWindowCreateDetail)
var signalWindowCreateLock sync.RWMutex

// WindowSignalCreateCallback is a callback function for a 'create' signal emitted from a Window.
type WindowSignalCreateCallback func(targetObject *Window)

/*
ConnectCreate connects the callback to the 'create' signal for the Window.

The returned value represents the connection, and may be passed to DisconnectCreate to remove it.
*/
func (recv *Window) ConnectCreate(callback WindowSignalCreateCallback) int {
	signalWindowCreateLock.Lock()
	defer signalWindowCreateLock.Unlock()

	signalWindowCreateId++
	instance := C.gpointer(recv.native)
	handlerID := C.Window_signal_connect_create(instance, C.gpointer(uintptr(signalWindowCreateId)))

	detail := signalWindowCreateDetail{callback, handlerID}
	signalWindowCreateMap[signalWindowCreateId] = detail

	return signalWindowCreateId
}

/*
DisconnectCreate disconnects a callback from the 'create' signal for the Window.

The connectionID should be a value returned from a call to ConnectCreate.
*/
func (recv *Window) DisconnectCreate(connectionID int) {
	signalWindowCreateLock.Lock()
	defer signalWindowCreateLock.Unlock()

	detail, exists := signalWindowCreateMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWindowCreateMap, connectionID)
}

//export window_createHandler
func window_createHandler(c_targetObject *C.GObject, data C.gpointer) {
	signalWindowCreateLock.RLock()
	defer signalWindowCreateLock.RUnlock()

	targetObject := WindowNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalWindowCreateMap[index].callback
	callback(targetObject)
}

type signalWindowDeactivateDetail struct {
	callback  WindowSignalDeactivateCallback
	handlerID C.gulong
}

var signalWindowDeactivateId int
var signalWindowDeactivateMap = make(map[int]signalWindowDeactivateDetail)
var signalWindowDeactivateLock sync.RWMutex

// WindowSignalDeactivateCallback is a callback function for a 'deactivate' signal emitted from a Window.
type WindowSignalDeactivateCallback func(targetObject *Window)

/*
ConnectDeactivate connects the callback to the 'deactivate' signal for the Window.

The returned value represents the connection, and may be passed to DisconnectDeactivate to remove it.
*/
func (recv *Window) ConnectDeactivate(callback WindowSignalDeactivateCallback) int {
	signalWindowDeactivateLock.Lock()
	defer signalWindowDeactivateLock.Unlock()

	signalWindowDeactivateId++
	instance := C.gpointer(recv.native)
	handlerID := C.Window_signal_connect_deactivate(instance, C.gpointer(uintptr(signalWindowDeactivateId)))

	detail := signalWindowDeactivateDetail{callback, handlerID}
	signalWindowDeactivateMap[signalWindowDeactivateId] = detail

	return signalWindowDeactivateId
}

/*
DisconnectDeactivate disconnects a callback from the 'deactivate' signal for the Window.

The connectionID should be a value returned from a call to ConnectDeactivate.
*/
func (recv *Window) DisconnectDeactivate(connectionID int) {
	signalWindowDeactivateLock.Lock()
	defer signalWindowDeactivateLock.Unlock()

	detail, exists := signalWindowDeactivateMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWindowDeactivateMap, connectionID)
}

//export window_deactivateHandler
func window_deactivateHandler(c_targetObject *C.GObject, data C.gpointer) {
	signalWindowDeactivateLock.RLock()
	defer signalWindowDeactivateLock.RUnlock()

	targetObject := WindowNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalWindowDeactivateMap[index].callback
	callback(targetObject)
}

type signalWindowDestroyDetail struct {
	callback  WindowSignalDestroyCallback
	handlerID C.gulong
}

var signalWindowDestroyId int
var signalWindowDestroyMap = make(map[int]signalWindowDestroyDetail)
var signalWindowDestroyLock sync.RWMutex

// WindowSignalDestroyCallback is a callback function for a 'destroy' signal emitted from a Window.
type WindowSignalDestroyCallback func(targetObject *Window)

/*
ConnectDestroy connects the callback to the 'destroy' signal for the Window.

The returned value represents the connection, and may be passed to DisconnectDestroy to remove it.
*/
func (recv *Window) ConnectDestroy(callback WindowSignalDestroyCallback) int {
	signalWindowDestroyLock.Lock()
	defer signalWindowDestroyLock.Unlock()

	signalWindowDestroyId++
	instance := C.gpointer(recv.native)
	handlerID := C.Window_signal_connect_destroy(instance, C.gpointer(uintptr(signalWindowDestroyId)))

	detail := signalWindowDestroyDetail{callback, handlerID}
	signalWindowDestroyMap[signalWindowDestroyId] = detail

	return signalWindowDestroyId
}

/*
DisconnectDestroy disconnects a callback from the 'destroy' signal for the Window.

The connectionID should be a value returned from a call to ConnectDestroy.
*/
func (recv *Window) DisconnectDestroy(connectionID int) {
	signalWindowDestroyLock.Lock()
	defer signalWindowDestroyLock.Unlock()

	detail, exists := signalWindowDestroyMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWindowDestroyMap, connectionID)
}

//export window_destroyHandler
func window_destroyHandler(c_targetObject *C.GObject, data C.gpointer) {
	signalWindowDestroyLock.RLock()
	defer signalWindowDestroyLock.RUnlock()

	targetObject := WindowNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalWindowDestroyMap[index].callback
	callback(targetObject)
}

type signalWindowMaximizeDetail struct {
	callback  WindowSignalMaximizeCallback
	handlerID C.gulong
}

var signalWindowMaximizeId int
var signalWindowMaximizeMap = make(map[int]signalWindowMaximizeDetail)
var signalWindowMaximizeLock sync.RWMutex

// WindowSignalMaximizeCallback is a callback function for a 'maximize' signal emitted from a Window.
type WindowSignalMaximizeCallback func(targetObject *Window)

/*
ConnectMaximize connects the callback to the 'maximize' signal for the Window.

The returned value represents the connection, and may be passed to DisconnectMaximize to remove it.
*/
func (recv *Window) ConnectMaximize(callback WindowSignalMaximizeCallback) int {
	signalWindowMaximizeLock.Lock()
	defer signalWindowMaximizeLock.Unlock()

	signalWindowMaximizeId++
	instance := C.gpointer(recv.native)
	handlerID := C.Window_signal_connect_maximize(instance, C.gpointer(uintptr(signalWindowMaximizeId)))

	detail := signalWindowMaximizeDetail{callback, handlerID}
	signalWindowMaximizeMap[signalWindowMaximizeId] = detail

	return signalWindowMaximizeId
}

/*
DisconnectMaximize disconnects a callback from the 'maximize' signal for the Window.

The connectionID should be a value returned from a call to ConnectMaximize.
*/
func (recv *Window) DisconnectMaximize(connectionID int) {
	signalWindowMaximizeLock.Lock()
	defer signalWindowMaximizeLock.Unlock()

	detail, exists := signalWindowMaximizeMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWindowMaximizeMap, connectionID)
}

//export window_maximizeHandler
func window_maximizeHandler(c_targetObject *C.GObject, data C.gpointer) {
	signalWindowMaximizeLock.RLock()
	defer signalWindowMaximizeLock.RUnlock()

	targetObject := WindowNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalWindowMaximizeMap[index].callback
	callback(targetObject)
}

type signalWindowMinimizeDetail struct {
	callback  WindowSignalMinimizeCallback
	handlerID C.gulong
}

var signalWindowMinimizeId int
var signalWindowMinimizeMap = make(map[int]signalWindowMinimizeDetail)
var signalWindowMinimizeLock sync.RWMutex

// WindowSignalMinimizeCallback is a callback function for a 'minimize' signal emitted from a Window.
type WindowSignalMinimizeCallback func(targetObject *Window)

/*
ConnectMinimize connects the callback to the 'minimize' signal for the Window.

The returned value represents the connection, and may be passed to DisconnectMinimize to remove it.
*/
func (recv *Window) ConnectMinimize(callback WindowSignalMinimizeCallback) int {
	signalWindowMinimizeLock.Lock()
	defer signalWindowMinimizeLock.Unlock()

	signalWindowMinimizeId++
	instance := C.gpointer(recv.native)
	handlerID := C.Window_signal_connect_minimize(instance, C.gpointer(uintptr(signalWindowMinimizeId)))

	detail := signalWindowMinimizeDetail{callback, handlerID}
	signalWindowMinimizeMap[signalWindowMinimizeId] = detail

	return signalWindowMinimizeId
}

/*
DisconnectMinimize disconnects a callback from the 'minimize' signal for the Window.

The connectionID should be a value returned from a call to ConnectMinimize.
*/
func (recv *Window) DisconnectMinimize(connectionID int) {
	signalWindowMinimizeLock.Lock()
	defer signalWindowMinimizeLock.Unlock()

	detail, exists := signalWindowMinimizeMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWindowMinimizeMap, connectionID)
}

//export window_minimizeHandler
func window_minimizeHandler(c_targetObject *C.GObject, data C.gpointer) {
	signalWindowMinimizeLock.RLock()
	defer signalWindowMinimizeLock.RUnlock()

	targetObject := WindowNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalWindowMinimizeMap[index].callback
	callback(targetObject)
}

type signalWindowMoveDetail struct {
	callback  WindowSignalMoveCallback
	handlerID C.gulong
}

var signalWindowMoveId int
var signalWindowMoveMap = make(map[int]signalWindowMoveDetail)
var signalWindowMoveLock sync.RWMutex

// WindowSignalMoveCallback is a callback function for a 'move' signal emitted from a Window.
type WindowSignalMoveCallback func(targetObject *Window)

/*
ConnectMove connects the callback to the 'move' signal for the Window.

The returned value represents the connection, and may be passed to DisconnectMove to remove it.
*/
func (recv *Window) ConnectMove(callback WindowSignalMoveCallback) int {
	signalWindowMoveLock.Lock()
	defer signalWindowMoveLock.Unlock()

	signalWindowMoveId++
	instance := C.gpointer(recv.native)
	handlerID := C.Window_signal_connect_move(instance, C.gpointer(uintptr(signalWindowMoveId)))

	detail := signalWindowMoveDetail{callback, handlerID}
	signalWindowMoveMap[signalWindowMoveId] = detail

	return signalWindowMoveId
}

/*
DisconnectMove disconnects a callback from the 'move' signal for the Window.

The connectionID should be a value returned from a call to ConnectMove.
*/
func (recv *Window) DisconnectMove(connectionID int) {
	signalWindowMoveLock.Lock()
	defer signalWindowMoveLock.Unlock()

	detail, exists := signalWindowMoveMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWindowMoveMap, connectionID)
}

//export window_moveHandler
func window_moveHandler(c_targetObject *C.GObject, data C.gpointer) {
	signalWindowMoveLock.RLock()
	defer signalWindowMoveLock.RUnlock()

	targetObject := WindowNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalWindowMoveMap[index].callback
	callback(targetObject)
}

type signalWindowResizeDetail struct {
	callback  WindowSignalResizeCallback
	handlerID C.gulong
}

var signalWindowResizeId int
var signalWindowResizeMap = make(map[int]signalWindowResizeDetail)
var signalWindowResizeLock sync.RWMutex

// WindowSignalResizeCallback is a callback function for a 'resize' signal emitted from a Window.
type WindowSignalResizeCallback func(targetObject *Window)

/*
ConnectResize connects the callback to the 'resize' signal for the Window.

The returned value represents the connection, and may be passed to DisconnectResize to remove it.
*/
func (recv *Window) ConnectResize(callback WindowSignalResizeCallback) int {
	signalWindowResizeLock.Lock()
	defer signalWindowResizeLock.Unlock()

	signalWindowResizeId++
	instance := C.gpointer(recv.native)
	handlerID := C.Window_signal_connect_resize(instance, C.gpointer(uintptr(signalWindowResizeId)))

	detail := signalWindowResizeDetail{callback, handlerID}
	signalWindowResizeMap[signalWindowResizeId] = detail

	return signalWindowResizeId
}

/*
DisconnectResize disconnects a callback from the 'resize' signal for the Window.

The connectionID should be a value returned from a call to ConnectResize.
*/
func (recv *Window) DisconnectResize(connectionID int) {
	signalWindowResizeLock.Lock()
	defer signalWindowResizeLock.Unlock()

	detail, exists := signalWindowResizeMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWindowResizeMap, connectionID)
}

//export window_resizeHandler
func window_resizeHandler(c_targetObject *C.GObject, data C.gpointer) {
	signalWindowResizeLock.RLock()
	defer signalWindowResizeLock.RUnlock()

	targetObject := WindowNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalWindowResizeMap[index].callback
	callback(targetObject)
}

type signalWindowRestoreDetail struct {
	callback  WindowSignalRestoreCallback
	handlerID C.gulong
}

var signalWindowRestoreId int
var signalWindowRestoreMap = make(map[int]signalWindowRestoreDetail)
var signalWindowRestoreLock sync.RWMutex

// WindowSignalRestoreCallback is a callback function for a 'restore' signal emitted from a Window.
type WindowSignalRestoreCallback func(targetObject *Window)

/*
ConnectRestore connects the callback to the 'restore' signal for the Window.

The returned value represents the connection, and may be passed to DisconnectRestore to remove it.
*/
func (recv *Window) ConnectRestore(callback WindowSignalRestoreCallback) int {
	signalWindowRestoreLock.Lock()
	defer signalWindowRestoreLock.Unlock()

	signalWindowRestoreId++
	instance := C.gpointer(recv.native)
	handlerID := C.Window_signal_connect_restore(instance, C.gpointer(uintptr(signalWindowRestoreId)))

	detail := signalWindowRestoreDetail{callback, handlerID}
	signalWindowRestoreMap[signalWindowRestoreId] = detail

	return signalWindowRestoreId
}

/*
DisconnectRestore disconnects a callback from the 'restore' signal for the Window.

The connectionID should be a value returned from a call to ConnectRestore.
*/
func (recv *Window) DisconnectRestore(connectionID int) {
	signalWindowRestoreLock.Lock()
	defer signalWindowRestoreLock.Unlock()

	detail, exists := signalWindowRestoreMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWindowRestoreMap, connectionID)
}

//export window_restoreHandler
func window_restoreHandler(c_targetObject *C.GObject, data C.gpointer) {
	signalWindowRestoreLock.RLock()
	defer signalWindowRestoreLock.RUnlock()

	targetObject := WindowNewFromC((unsafe.Pointer)(c_targetObject))

	index := int(uintptr(data))
	callback := signalWindowRestoreMap[index].callback
	callback(targetObject)
}
