...
func Fn_gdk_cairo_draw_from_gl(cr *cairo.Context, window *Window, source int, sourceType int, bufferScale int, x int, y int, width int, height int) {
	sys_cr := unsafe.Pointer(cr)
	sys_window := unsafe.Pointer(window)
	sys_source := int(source)
	sys_sourceType := int(sourceType)
	sys_bufferScale := int(bufferScale)
	sys_x := int(x)
	sys_y := int(y)
	sys_width := int(width)
...
func Fn_gdk_cairo_get_drawing_context(cr *cairo.Context) {
	sys_cr := unsafe.Pointer(cr)
}
...
func Fn_gdk_cairo_set_source_rgba(cr *cairo.Context, rgba *RGBA) {
	sys_cr := unsafe.Pointer(cr)
	sys_rgba := unsafe.Pointer(rgba)
}

func Fn_gdk_cairo_set_source_window(cr *cairo.Context, window *Window, x float64, y float64) {
	sys_cr := unsafe.Pointer(cr)
	sys_window := unsafe.Pointer(window)
	sys_x := float64(x)
	sys_y := float64(y)
...
func Fn_gdk_cairo_surface_create_from_pixbuf(pixbuf *gdkpixbuf.Pixbuf, scale int, forWindow *Window) {
	sys_pixbuf := unsafe.Pointer(pixbuf)
	sys_scale := int(scale)
	sys_forWindow := unsafe.Pointer(forWindow)
}
...
func Fn_gdk_drag_begin_from_point(window *Window, device *Device, targets *glib.List, xRoot int, yRoot int) {
	sys_window := unsafe.Pointer(window)
	sys_device := unsafe.Pointer(device)
	sys_targets := unsafe.Pointer(targets)
	sys_xRoot := int(xRoot)
...
func Fn_gdk_drag_drop_done(context *DragContext, success bool) {
	sys_context := unsafe.Pointer(context)
...
func Fn_gdk_events_get_angle(event1 *Event, event2 *Event) {
	sys_event1 := unsafe.Pointer(event1)
	sys_event2 := unsafe.Pointer(event2)
}

func Fn_gdk_events_get_center(event1 *Event, event2 *Event) {
	sys_event1 := unsafe.Pointer(event1)
	sys_event2 := unsafe.Pointer(event2)
}

func Fn_gdk_events_get_distance(event1 *Event, event2 *Event) {
	sys_event1 := unsafe.Pointer(event1)
	sys_event2 := unsafe.Pointer(event2)
}
...
func Fn_gdk_offscreen_window_get_embedder(window *Window) {
	sys_window := unsafe.Pointer(window)
}
...
func Fn_gdk_offscreen_window_set_embedder(window *Window, embedder *Window) {
	sys_window := unsafe.Pointer(window)
	sys_embedder := unsafe.Pointer(embedder)
}

func Fn_gdk_pango_context_get() {}

func Fn_gdk_pango_context_get_for_display(display *Display) {
