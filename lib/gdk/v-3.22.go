...
func CairoGetDrawingContext(cr *cairo.Context) unsafe.Pointer {
	sys_cr := cr.ToC()
	ret := gdk.Fn_gdk_cairo_get_drawing_context(sys_cr)
...
func CairoSurfaceCreateFromPixbuf(pixbuf *gdkpixbuf.Pixbuf, scale int, forWindow *Window) unsafe.Pointer {
	sys_pixbuf := pixbuf.ToC()
	sys_scale := scale
	sys_forWindow := forWindow.ToC()
	ret := gdk.Fn_gdk_cairo_surface_create_from_pixbuf(sys_pixbuf, sys_scale, sys_forWindow)
...
func DragBeginFromPoint(window *Window, device *Device, targets *glib.List, xRoot int, yRoot int) unsafe.Pointer {
	sys_window := window.ToC()
	sys_device := device.ToC()
	sys_targets := targets.ToC()
	sys_xRoot := xRoot
	sys_yRoot := yRoot
	ret := gdk.Fn_gdk_drag_begin_from_point(sys_window, sys_device, sys_targets, sys_xRoot, sys_yRoot)
...
func PangoContextGetForDisplay(display *Display) unsafe.Pointer {
	sys_display := display.ToC()
	ret := gdk.Fn_gdk_pango_context_get_for_display(sys_display)
