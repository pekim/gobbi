...
func CairoGetDrawingContext(cr *cairo.Context) {
	sys_cr := cr.ToC()
	gdk.Fn_gdk_cairo_get_drawing_context(sys_cr)
}
...
func CairoSurfaceCreateFromPixbuf(pixbuf *gdkpixbuf.Pixbuf, scale int, forWindow *Window) {
	...
	gdk.Fn_gdk_cairo_surface_create_from_pixbuf(sys_pixbuf, sys_scale, sys_forWindow)
}
...
func DragBeginFromPoint(window *Window, device *Device, targets *glib.List, xRoot int, yRoot int) {
	...
	sys_xRoot := xRoot
	sys_yRoot := yRoot
	gdk.Fn_gdk_drag_begin_from_point(sys_window, sys_device, sys_targets, sys_xRoot, sys_yRoot)
}
...
func PangoContextGetForDisplay(display *Display) {
	...
	gdk.Fn_gdk_pango_context_get_for_display(sys_display)
}
