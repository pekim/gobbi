...
func Fn_g_enum_to_string(gEnumType uint64, value int) {
	sys_gEnumType := uint64(gEnumType)
	sys_value := int(value)
}
...
func Fn_g_flags_to_string(flagsType uint64, value uint) {
...
func Fn_g_param_spec_gtype(name string, nick string, blurb string, isAType uint64, flags int) {
	sys_name := string(name)
	sys_nick := string(nick)
	sys_blurb := string(blurb)
...
func Fn_g_param_spec_variant(name string, nick string, blurb string, type_ *glib.VariantType, defaultValue *glib.Variant, flags int) {
	sys_name := string(name)
	sys_nick := string(nick)
	sys_blurb := string(blurb)
...
func Fn_g_signal_accumulator_first_wins(ihint *SignalInvocationHint, returnAccu *Value, handlerReturn *Value, dummy unsafe.Pointer) {
...
func Fn_g_type_get_instance_count(type_ uint64) {
	sys_type_ := uint64(type_)
}
...
func Fn_g_type_get_type_registration_serial() {}
