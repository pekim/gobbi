...
func EnumToString(gEnumType uint64, value int) {
	sys_gEnumType := gEnumType
	sys_value := value
	gobject.Fn_g_enum_to_string(sys_gEnumType, sys_value)
}
...
func FlagsToString(flagsType uint64, value uint) {
	sys_flagsType := flagsType
	sys_value := value
	gobject.Fn_g_flags_to_string(sys_flagsType, sys_value)
}
...
func ParamSpecVariant_(name string, nick string, blurb string, type_ *glib.VariantType, defaultValue *glib.Variant, flags int) {
	...
	sys_flags := flags
	gobject.Fn_g_param_spec_variant(sys_name, sys_nick, sys_blurb, sys_type_, sys_defaultValue, sys_flags)
}
...
func SignalAccumulatorFirstWins(ihint *SignalInvocationHint, returnAccu *Value, handlerReturn *Value, dummy unsafe.Pointer) {
	...
	sys_dummy := dummy
	gobject.Fn_g_signal_accumulator_first_wins(sys_ihint, sys_returnAccu, sys_handlerReturn, sys_dummy)
}
...
func TypeGetInstanceCount(type_ uint64) {
	sys_type_ := type_
	gobject.Fn_g_type_get_instance_count(sys_type_)
}
...
func TypeGetTypeRegistrationSerial() {
	gobject.Fn_g_type_get_type_registration_serial()
}
