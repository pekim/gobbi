...
// +build atk_2.30 atk_2.32 atk_2.34
...
func Fn_atk_component_scroll_to(paramInstance unsafe.Pointer, param0 int) bool {
	cValueInstance := (*C.AtkComponent)(unsafe.Pointer(paramInstance))

	cValue0 := (C.AtkScrollType)(param0)

	ret := C.atk_component_scroll_to(cValueInstance, cValue0)

	return toGoBool(ret)
}

func Fn_atk_component_scroll_to_point(paramInstance unsafe.Pointer, param0 int, param1 int, param2 int) bool {
	cValueInstance := (*C.AtkComponent)(unsafe.Pointer(paramInstance))

	cValue0 := (C.AtkCoordType)(param0)

	cValue1 := (C.gint)(param1)

	cValue2 := (C.gint)(param2)

	ret := C.atk_component_scroll_to_point(cValueInstance, cValue0, cValue1, cValue2)
