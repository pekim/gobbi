...
/*
	...
	void websocketconnection_pongHandler(GObject *, GBytes *, gpointer);

	static gulong WebsocketConnection_signal_connect_pong(gpointer instance, gpointer data) {
		return g_signal_connect(instance, "pong", G_CALLBACK(websocketconnection_pongHandler), data);
	}
...
/*
	...
	static void _soup_xmlrpc_message_set_fault(SoupMessage* msg, int fault_code, const char* fault_format) {
		return soup_xmlrpc_message_set_fault(msg, fault_code, fault_format);
    }
*/
/*
...
// CanAuthenticate is a wrapper around the C function soup_auth_can_authenticate.
func (recv *Auth) CanAuthenticate() bool {
	retC := C.soup_auth_can_authenticate((*C.SoupAuth)(recv.native))
	retGo := retC == C.TRUE
...
// ClearCachedCredentials is a wrapper around the C function soup_auth_manager_clear_cached_credentials.
func (recv *AuthManager) ClearCachedCredentials() {
	C.soup_auth_manager_clear_cached_credentials((*C.SoupAuthManager)(recv.native))
...
// Unsupported : soup_session_connect_async : unsupported parameter progress_callback : no type generator for SessionConnectProgressCallback (SoupSessionConnectProgressCallback) for param progress_callback

// ConnectFinish is a wrapper around the C function soup_session_connect_finish.
func (recv *Session) ConnectFinish(result *gio.AsyncResult) (*gio.IOStream, error) {
	c_result := (*C.GAsyncResult)(result.ToC())

	var cThrowableError *C.GError

	retC := C.soup_session_connect_finish((*C.SoupSession)(recv.native), c_result, &cThrowableError)
...
type signalWebsocketConnectionPongDetail struct {
	callback  WebsocketConnectionSignalPongCallback
	handlerID C.gulong
}

var signalWebsocketConnectionPongId int
var signalWebsocketConnectionPongMap = make(map[int]signalWebsocketConnectionPongDetail)
var signalWebsocketConnectionPongLock sync.RWMutex

// WebsocketConnectionSignalPongCallback is a callback function for a 'pong' signal emitted from a WebsocketConnection.
type WebsocketConnectionSignalPongCallback func(targetObject *WebsocketConnection, message *glib.Bytes)

/*
ConnectPong connects the callback to the 'pong' signal for the WebsocketConnection.

The returned value represents the connection, and may be passed to DisconnectPong to remove it.
*/
func (recv *WebsocketConnection) ConnectPong(callback WebsocketConnectionSignalPongCallback) int {
	signalWebsocketConnectionPongLock.Lock()
	defer signalWebsocketConnectionPongLock.Unlock()

	signalWebsocketConnectionPongId++
	instance := C.gpointer(recv.native)
	handlerID := C.WebsocketConnection_signal_connect_pong(instance, C.gpointer(uintptr(signalWebsocketConnectionPongId)))

	detail := signalWebsocketConnectionPongDetail{callback, handlerID}
	signalWebsocketConnectionPongMap[signalWebsocketConnectionPongId] = detail

	return signalWebsocketConnectionPongId
}

/*
DisconnectPong disconnects a callback from the 'pong' signal for the WebsocketConnection.

The connectionID should be a value returned from a call to ConnectPong.
*/
func (recv *WebsocketConnection) DisconnectPong(connectionID int) {
	signalWebsocketConnectionPongLock.Lock()
	defer signalWebsocketConnectionPongLock.Unlock()

	detail, exists := signalWebsocketConnectionPongMap[connectionID]
	if !exists {
		return
	}
	...
	instance := C.gpointer(recv.native)
	C.g_signal_handler_disconnect(instance, detail.handlerID)
	delete(signalWebsocketConnectionPongMap, connectionID)
}

//export websocketconnection_pongHandler
func websocketconnection_pongHandler(c_targetObject *C.GObject, c_message *C.GBytes, data C.gpointer) {
	signalWebsocketConnectionPongLock.RLock()
	defer signalWebsocketConnectionPongLock.RUnlock()
	...
	index := int(uintptr(data))
	callback := signalWebsocketConnectionPongMap[index].callback
	callback(targetObject, message)
}
...
// GetKeepaliveInterval is a wrapper around the C function soup_websocket_connection_get_keepalive_interval.
func (recv *WebsocketConnection) GetKeepaliveInterval() uint32 {
	retC := C.soup_websocket_connection_get_keepalive_interval((*C.SoupWebsocketConnection)(recv.native))
	retGo := (uint32)(retC)

	return retGo
}

// GetMaxIncomingPayloadSize is a wrapper around the C function soup_websocket_connection_get_max_incoming_payload_size.
func (recv *WebsocketConnection) GetMaxIncomingPayloadSize() uint64 {
	retC := C.soup_websocket_connection_get_max_incoming_payload_size((*C.SoupWebsocketConnection)(recv.native))
	retGo := (uint64)(retC)
...
// SetKeepaliveInterval is a wrapper around the C function soup_websocket_connection_set_keepalive_interval.
func (recv *WebsocketConnection) SetKeepaliveInterval(interval uint32) {
	c_interval := (C.guint)(interval)

	C.soup_websocket_connection_set_keepalive_interval((*C.SoupWebsocketConnection)(recv.native), c_interval)

	return
}

// SetMaxIncomingPayloadSize is a wrapper around the C function soup_websocket_connection_set_max_incoming_payload_size.
func (recv *WebsocketConnection) SetMaxIncomingPayloadSize(maxIncomingPayloadSize uint64) {
	c_max_incoming_payload_size := (C.guint64)(maxIncomingPayloadSize)

	C.soup_websocket_connection_set_max_incoming_payload_size((*C.SoupWebsocketConnection)(recv.native), c_max_incoming_payload_size)
...
const LOGGER_LEVEL string = C.SOUP_LOGGER_LEVEL
const LOGGER_MAX_BODY_SIZE string = C.SOUP_LOGGER_MAX_BODY_SIZE
...
// XmlrpcBuildRequest is a wrapper around the C function soup_xmlrpc_build_request.
func XmlrpcBuildRequest(methodName string, params *glib.Variant) (string, error) {
	...
	c_params := (*C.GVariant)(C.NULL)
	if params != nil {
		c_params = (*C.GVariant)(params.ToC())
	}

	var cThrowableError *C.GError

	retC := C.soup_xmlrpc_build_request(c_method_name, c_params, &cThrowableError)
	retGo := C.GoString(retC)
	defer C.free(unsafe.Pointer(retC))
...
// XmlrpcBuildResponse is a wrapper around the C function soup_xmlrpc_build_response.
func XmlrpcBuildResponse(value *glib.Variant) (string, error) {
	...
	retC := C.soup_xmlrpc_build_response(c_value, &cThrowableError)
	retGo := C.GoString(retC)
	defer C.free(unsafe.Pointer(retC))
...
// XmlrpcMessageNew is a wrapper around the C function soup_xmlrpc_message_new.
func XmlrpcMessageNew(uri string, methodName string, params *glib.Variant) (*Message, error) {
	c_uri := C.CString(uri)
	defer C.free(unsafe.Pointer(c_uri))
	...
	retC := C.soup_xmlrpc_message_new(c_uri, c_method_name, c_params, &cThrowableError)
	retGo := MessageNewFromC(unsafe.Pointer(retC))
...
// XmlrpcMessageSetFault is a wrapper around the C function soup_xmlrpc_message_set_fault.
func XmlrpcMessageSetFault(msg *Message, faultCode int32, faultFormat string, args ...interface{}) {
	c_msg := (*C.SoupMessage)(C.NULL)
	if msg != nil {
		c_msg = (*C.SoupMessage)(msg.ToC())
	}
	...
	C._soup_xmlrpc_message_set_fault(c_msg, c_fault_code, c_fault_format)

	return
}

// XmlrpcMessageSetResponse is a wrapper around the C function soup_xmlrpc_message_set_response.
func XmlrpcMessageSetResponse(msg *Message, value *glib.Variant) (bool, error) {
	c_msg := (*C.SoupMessage)(C.NULL)
	if msg != nil {
		c_msg = (*C.SoupMessage)(msg.ToC())
	}
	...
	retC := C.soup_xmlrpc_message_set_response(c_msg, c_value, &cThrowableError)
	retGo := retC == C.TRUE
...
// XmlrpcParseRequest is a wrapper around the C function soup_xmlrpc_parse_request.
func XmlrpcParseRequest(methodCall string, length int32) (string, *XMLRPCParams, error) {
	...
	var c_params *C.SoupXMLRPCParams

	var cThrowableError *C.GError

	retC := C.soup_xmlrpc_parse_request(c_method_call, c_length, &c_params, &cThrowableError)
	retGo := C.GoString(retC)
	defer C.free(unsafe.Pointer(retC))
	...
	params := XMLRPCParamsNewFromC(unsafe.Pointer(c_params))

	return retGo, params, goError
}

// XmlrpcParseResponse is a wrapper around the C function soup_xmlrpc_parse_response.
func XmlrpcParseResponse(methodResponse string, length int32, signature string) (*glib.Variant, error) {
	...
	retC := C.soup_xmlrpc_parse_response(c_method_response, c_length, c_signature, &cThrowableError)
...
// XmlrpcVariantGetDatetime is a wrapper around the C function soup_xmlrpc_variant_get_datetime.
func XmlrpcVariantGetDatetime(variant *glib.Variant) (*Date, error) {
	...
	retC := C.soup_xmlrpc_variant_get_datetime(c_variant, &cThrowableError)
...
// XmlrpcVariantNewDatetime is a wrapper around the C function soup_xmlrpc_variant_new_datetime.
func XmlrpcVariantNewDatetime(date *Date) *glib.Variant {
	c_date := (*C.SoupDate)(C.NULL)
	if date != nil {
		c_date = (*C.SoupDate)(date.ToC())
	}

	retC := C.soup_xmlrpc_variant_new_datetime(c_date)
	retGo := glib.VariantNewFromC(unsafe.Pointer(retC))
...
// XMLRPCParams is a wrapper around the C record SoupXMLRPCParams.
type XMLRPCParams struct {
	native *C.SoupXMLRPCParams
}

func XMLRPCParamsNewFromC(u unsafe.Pointer) *XMLRPCParams {
	c := (*C.SoupXMLRPCParams)(u)
	if c == nil {
		return nil
	}

	g := &XMLRPCParams{native: c}

	return g
}

func (recv *XMLRPCParams) ToC() unsafe.Pointer {

	return (unsafe.Pointer)(recv.native)
}

// Equals compares this XMLRPCParams with another XMLRPCParams, and returns true if they represent the same GObject.
func (recv *XMLRPCParams) Equals(other *XMLRPCParams) bool {
	return other.ToC() == recv.ToC()
}

// Free is a wrapper around the C function soup_xmlrpc_params_free.
func (recv *XMLRPCParams) Free() {
	C.soup_xmlrpc_params_free((*C.SoupXMLRPCParams)(recv.native))

	return
}

// Parse is a wrapper around the C function soup_xmlrpc_params_parse.
func (recv *XMLRPCParams) Parse(signature string) (*glib.Variant, error) {
	c_signature := C.CString(signature)
	defer C.free(unsafe.Pointer(c_signature))

	var cThrowableError *C.GError

	retC := C.soup_xmlrpc_params_parse((*C.SoupXMLRPCParams)(recv.native), c_signature, &cThrowableError)
	retGo := glib.VariantNewFromC(unsafe.Pointer(retC))
